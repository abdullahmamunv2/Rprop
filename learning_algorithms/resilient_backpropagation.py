...
def resilient_backpropagation(network, trainingset, testset, cost_function, ERROR_LIMIT=1e-3, max_iterations = (), weight_step_max = 50., weight_step_min = 0., start_step = 0.5, learn_max = 1.2, learn_min = 0.5, print_rate = 1000, save_trained_network = False ):
    ...
    # Storing the current / previous weight step size delta(t-1)
    deltaUpdate                = [ np.full( weight_layer.shape, start_step ) for weight_layer in network.weights ]


    
    # Storing the current / previous weight update  deltaW(t-1)
    cWeights                         = [  np.ones(shape=weight_layer.shape) for weight_layer in network.weights ]

    
    # Storing the previous derivative   dE/dW(t-1)
    previous_error_rate              = [ 1 ] * len( network.weights )
    ...
    while error > ERROR_LIMIT and epoch < max_iterations:
        ...
        for i in layer_indexes:
            ...
            diffs            = np.multiply( dEdW, previous_error_rate[i] )
            ...
            if np.any(pos_indexes):
                ...
                deltaUpdate[i][pos_indexes] = np.minimum( deltaUpdate[i][pos_indexes] * learn_max, weight_step_max )
                
                # Calculate the weight step direction
                cWeights[i][pos_indexes] = np.multiply( -np.sign( dEdW[pos_indexes] ), deltaUpdate[i][pos_indexes] )
                
                # Apply the weight deltas
                network.weights[i][ pos_indexes ] += cWeights[i][pos_indexes]
            
            # negative
            if np.any(neg_indexes):
                deltaUpdate[i][neg_indexes] = np.maximum( deltaUpdate[i][neg_indexes] * learn_min, weight_step_min )
                
                if error > prev_error:
                    # iRprop+ version of resilient backpropagation
                    network.weights[i][ neg_indexes ] -= cWeights[i][neg_indexes] # backtrack
                
                dEdW[ neg_indexes ] = 0
            
            # zeros
            if np.any(zero_indexes):
                cWeights[i][zero_indexes] = np.multiply( -np.sign( dEdW[zero_indexes] ), deltaUpdate[i][zero_indexes] )
                network.weights[i][ zero_indexes ] += cWeights[i][zero_indexes]
            
            # Store the previous weight step
            previous_error_rate[i] = dEdW
